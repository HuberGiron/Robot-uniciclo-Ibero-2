...
while (True):
    ...
    if len(points)>0:
        ...
        t[i]=i
        x[i]=robot[0][0]-(resolucionx/2) #Obtenemos valor X de QR robot 
        y[i]=robot[0][1]-(resoluciony/2) #Obtenemos valor Y de QR robot 
        th[i]=robot[0][2] #Obtenemos valor Th de robot 
