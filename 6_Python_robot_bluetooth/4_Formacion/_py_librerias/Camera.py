...
def draw_aruco(frame, topLeft, topRight, bottomLeft, bottomRight, MidP, X, Y,angle,markerID,resolucionx,resoluciony):
  ...
  cv2.putText(frame, "["+str(MidP[0][0]-(resolucionx/2))+" ,"+str(MidP[0][1]-(resoluciony/2))+"]", (MidP[0][0], MidP[0][1] - 150),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,204,204),2,cv2.LINE_AA )
  cv2.putText(frame, str(f'ID: {markerID}'), (MidP[0][0] , MidP[0][1] - 100), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 204, 204), 2, cv2.LINE_AA )
...
def draw_punto(frame,text,color, posx, posy, resolucionx,resoluciony):
  #Se imprime la información del texto 
  cv2.putText(frame,text,(posx+15,posy+15), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color,2, cv2.LINE_AA)
  cv2.putText(frame,str(posx-resolucionx/2)+","+str(posy-(resoluciony/2)),(posx+20,posy+50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2, cv2.LINE_AA)
  cv2.circle(frame,(posx,posy),7,color,5)
